...
This reference directory lists all available agent skills and slash commands installed globally and locally in this project, along with their scope, description, and typical triggers.

---

## 📦 Installed Skills & Commands Table

| Slash Command / Skill Name | Scope | Icon | Description |
| :--- | :---: | :-: | :--- |
| **`/ponytail`** | `Global` | 👱‍♀️ | Switch ponytail intensity level (`lite` / `full` / `ultra` / `off`). |
...
| **`/graphify`** | `Global` | 🕸️ | Build a navigable knowledge graph (`graphify-out/`) on a local folder or Git repo. |
| **`/graphify query "<q>"`** | `Global` | 🔎 | Query the knowledge graph using BFS/DFS traversals. |
| **`/graphify path "<A>" "<B>"`** | `Global` | 🛣️ | Find the shortest structural path between two nodes/concepts. |
...
| **`/improve-codebase-architecture`** | `Local` | 🏛️ | Scan codebase for architectural refactoring opportunities and generate reports. |
...
| **`/skill-creator`** | `Local` | 🏗️ | Create, benchmark, and optimize custom agent skills. |
...
* **⚡ Slash Commands**: Type any command directly in your prompt to trigger specialized workflows immediately (e.g., `/ponytail lite` or `/tdd`).
* **🧠 Context Injection**: When triggered, the agent automatically loads the corresponding `SKILL.md` instructions into the active context window.
